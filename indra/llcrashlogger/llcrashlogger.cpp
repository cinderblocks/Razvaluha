...
{
    std::string msg = "Request to " + response->getRequestURL() + " succeeded";
	if (content.has("message")) msg += ": " + content["message"].asString();
	LL_DEBUGS("CRASHREPORT") << msg << LL_ENDL;
    gBreak = true;
    gSent = true;

	if (content.has("report_id"))
		static_cast<LLCrashLogger*>(LLCrashLogger::instance())->getCrashSettings().setS32("CrashReportID", content["report_id"].asInteger());
}
...
LLCrashLogger::LLCrashLogger() :
	...
	mAltCrashHost("https://app.alchemyviewer.org/report/"),
...
void LLCrashLogger::gatherFiles()
{
	...
	else
	{
		...
		mFileMap["SecondLifeLog"] = gDirUtilp->getExpandedFilename(LL_PATH_LOGS,"Singularity.log");
...
std::string LLCrashLogger::loadCrashURLSetting()
{
	...
	mCrashBehavior = mCrashSettings.getS32("CrashSubmitBehavior");
