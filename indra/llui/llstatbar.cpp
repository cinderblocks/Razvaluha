...
:	LLView(p),
	...
	mSetting(p.setting().empty() ? nullptr : LLUI::sConfigGroup->getControl(p.setting())),
	mStatType(STAT_NONE),
	mLabel(p.label),
	...
	mAutoScaleMin(!p.bar_min.isProvided())
{
	if (mSetting)
	{
		const auto& mode = mSetting->get().asInteger();
		mDisplayBar = (mode & STAT_BAR_FLAG);
		mDisplayHistory = (mode & STAT_HISTORY_FLAG);
	}
...
BOOL LLStatBar::handleToolTip(S32 x, S32 y, std::string& message, LLRect* sticky_rect)
{
	switch(mStatType)
	{
	case STAT_COUNT:
		message = mStat.countStatp->getDescription();
		break;
	case STAT_EVENT:
		message = mStat.eventStatp->getDescription();
		break;
	case STAT_SAMPLE:
		message = mStat.sampleStatp->getDescription();
		break;
	case STAT_MEM:
		message = mStat.memStatp->getDescription();
		break;
	default:
		return false;
	}
...
BOOL LLStatBar::handleMouseDown(S32 x, S32 y, MASK mask)
{
	...
	if (!handled)
	{
		...
		{
			S32 mode = 0;
			if (mDisplayBar) mode |= STAT_BAR_FLAG;
			if (mDisplayHistory) mode |= STAT_HISTORY_FLAG;
			mSetting->set(mode);
		}
	}
	return TRUE;
...
void LLStatBar::draw()
{
	...
	if (mDisplayBar && mStat.valid)
	{
		...
		{
			static LLUIColor main_bar_color = LLColor4(1.f, 0.f, 0.f, 1.f);
			static LLUIColor main_bar_color_25 = LLColor4(1.f, 0.f, 0.f, 0.25f);
			// draw min and max
			...
			// draw mean bar
			{
				static LLUIColor mean_bar_color = LLColor4(0.f, 1.f, 0.f, 1.f);
