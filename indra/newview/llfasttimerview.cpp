...
:	LLFloater(name, rect, LLStringUtil::null, FALSE, 1, 1, FALSE, FALSE, TRUE),
...
static void get_tooltip(std::string& tooltip, BlockTimerStatHandle& timer, S32 history_index, PeriodicRecording& frame_recording)
{
	if (history_index == 0)
	{
...
BOOL LLFastTimerView::handleToolTip(S32 x, S32 y, std::string& tooltip, LLRect* screen_rect)
{
	if(mPauseHistory && mBarRect.pointInRect(x, y))
	{
		...
		if (mHoverTimer)
		{
			...
			get_tooltip(tooltip, *mHoverTimer, mHoverBarIndex > 0 ? mScrollIndex + mHoverBarIndex : 0, mRecording);
	...
	else
	{
		...
		if (x < mBarRect.mLeft) 
		{
			...
			if (idp)
			{
				get_tooltip(tooltip, *idp, 0, mRecording);
	...
	return LLFloater::handleToolTip(x, y, tooltip, screen_rect);
}
