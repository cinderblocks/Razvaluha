...
{
	for(timer_tree_iterator_t it = begin_timer_tree(LLFastTimer::NamedTimer::getRootTimeBlock());
		it != end_timer_tree();
		++it)
	{
...
void LLFastTimerView::draw()
{
	...
	sTimerColors[&LLFastTimer::NamedTimer::getRootTimeBlock()] = LLColor4::grey;
	...
	{	
		...
		if (!LLFastTimer::sPauseHistory)
		{
			U64 ticks = LLFastTimer::NamedTimer::getRootTimeBlock().getHistoricalCount(mScrollIndex);
		...
		else if (mDisplayMode == 2)
		{
			...
			{
				U64 ticks = LLFastTimer::NamedTimer::getRootTimeBlock().getHistoricalCount(j);
