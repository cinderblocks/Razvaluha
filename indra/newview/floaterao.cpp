...
BOOL LLFloaterAO::postBuild()
{
	...
	auto ao = AOSystem::instanceExists() ? AOSystem::getInstance() : nullptr;
...
void LLFloaterAO::onSpinnerCommit() const
{
	if (AOSystem::instanceExists())
		AOSystem::instance().mAOStandTimer.reset();
}
...
void LLFloaterAO::onComboBoxCommit(LLUICtrl* ctrl) const
{
	if (LLComboBox* box = (LLComboBox*)ctrl)
	{
		...
		if (auto ao = AOSystem::instanceExists() ? AOSystem::getInstance() : nullptr)
		{
