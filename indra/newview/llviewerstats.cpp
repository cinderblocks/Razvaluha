...
void output_statistics(void*)
{
	...
	for (S32 i = 0; i < 256; ++i)
	{
		if (obj_counts[i])
		{
			LL_CONT << LLPrimitive::pCodeToString(i) << ':' << obj_counts[i] << '\n';
		}
	}
	LL_CONT << LL_ENDL;
}
...
{
	gTotalWorldData += S32Bytes(gVLManager.getTotalBytes());
	...
	static const LLCachedControl<bool> render_vbo_enable(gSavedSettings, "RenderVBOEnable");
	static const LLCachedControl<F32> render_far_clip(gSavedSettings, "RenderFarClip");
	static const LLCachedControl<bool> is_bubble(gSavedSettings, "UseTypingBubbles");
	static const LLCachedControl<bool> local_lights("RenderLocalLights");
	sample(LLStatViewer::ENABLE_VBO,      (F64)render_vbo_enable);
	sample(LLStatViewer::LIGHTING_DETAIL, local_lights ? 1.0 : 0.0);
	...
	F64Bits layer_bits = S32Bits(gVLManager.getLandBits() + gVLManager.getWindBits() + gVLManager.getCloudBits());
...
void send_stats()
{
	...
	S32 shader_level = 0;
	static const LLCachedControl<bool> basic_shaders("VertexShaderEnable");
