...
{
	if (!gFloaterWorldMap) gFloaterWorldMap = new LLFloaterWorldMap;
	return gFloaterWorldMap;
}
...
// [RLVa:KB] - Checked: 2009-07-05 (RLVa-1.0.0c)
	if (gRlvHandler.hasBehaviour(RLV_BHVR_SHOWWORLDMAP)) return;
// [/RLVa:KB]
	LLFloaterWorldMap* map = LLFloaterWorldMap::getInstance();
	BOOL was_visible = map->getVisible();

	map->mIsClosing = FALSE;
	map->open();

	LLWorldMapView* map_panel;
	map_panel = (LLWorldMapView*)map->mPanel;
	...
	if (!was_visible)
	{
		...
		map->getChild<LLUICtrl>("location")->setFocus( TRUE);
		gFocusMgr.triggerFocusFlash();

		map->buildAvatarIDList();
		map->buildLandmarkIDLists();

		// If nothing is being tracked, set flag so the user position will be found
		map->mSetToUserPosition = ( LLTracker::getInstance()->getTrackingStatus() == LLTracker::TRACKING_NOTHING );
	}
	
	if (center_on_target)
	{
		map->centerOnTarget(FALSE);
	}
}
...
{
	LLFloaterWorldMap* map = getInstance();
	if (!map->getVisible()) show(false);
	else
	{
		map->mIsClosing = true;
		map->close();
...
void LLFloaterWorldMap::draw()
{
	...
	const LLColor4& map_track_disabled_color = gDisabledTrackColor;
