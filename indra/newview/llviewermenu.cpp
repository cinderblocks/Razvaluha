...

{
		...
		sub->addChild(new LLMenuItemCallGL("Message Builder", handle_open_message_builder, nullptr));
		sub->addChild(new LLMenuItemCallGL("Message Rewriter", handle_open_message_rewriter, nullptr));
...
void handle_open_message_log(void*)
{
	LLFloaterMessageLog::showInstance();
}

void handle_open_message_builder(void*)
{
	LLFloaterMessageBuilder::show(LLStringUtil::null);
}

void handle_open_message_rewriter(void*)
{
	LLFloaterMessageRewriter::showInstance();
}
...
void handle_dump_archetype_xml(void *)
{
	...
	avatar->dumpArchetypeXML(LLStringUtil::null, false);
}
...
const std::string BUY_CURRENCY_URL = "http://secondlife.com/app/currency/";
...
void dump_diag_info(void*) { LLPolyMesh::dumpDiagInfo(); }
...
void init_meshes_and_morphs_menu()
{
	...
	menu->setCanTearOff(TRUE);
	menu->addChild(new LLMenuItemCallGL("Dump Avatar Mesh Info", dump_diag_info));
	menu->addSeparator();
		...
		info_iter != mesh_info.end(); ++info_iter)
	{
			...
			lod_iter != lod_mesh.end(); ++lod_iter)
		{
			...
			const auto& morph_list = mesh_shared->getMorphList();

			LLMenuGL* lod_menu = new LLMenuGL(caption);
			lod_menu->addChild(new LLMenuItemCallGL("Save LLM", handle_mesh_save_llm, nullptr, (void*) mesh_shared));

			LLMenuGL* action_menu = new LLMenuGL("Base Mesh");
			action_menu->addChild(new LLMenuItemCallGL("Save OBJ", handle_mesh_save_obj, nullptr, (void*) mesh_shared));
			...
			if (lod == 0)
			{
				...
				action_menu->addChild(new LLMenuItemCallGL("Load OBJ", handle_mesh_load_obj, nullptr, (void*) mesh_shared));
			}
			...
			action_menu->addChild(new LLMenuItemCallGL("Save OBJ", handle_mesh_save_current_obj, nullptr, (void*) mesh_shared));
			...
			{
				std::string const& morph_name = morph->getName();;
				LLPolyMorphData* morph_data = morph;

				action_menu = new LLMenuGL(morph_name);

				action_menu->addChild(new LLMenuItemCallGL("Save OBJ", handle_morph_save_obj, nullptr, (void*) morph_data));
				action_menu->addChild(new LLMenuItemCallGL("Load OBJ", handle_morph_load_obj, nullptr, (void*) morph_data));
