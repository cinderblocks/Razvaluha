...
void auth_handler(const std::string& url, const LLSD& data)
{
	const auto status = LLCoreHttpUtil::HttpCoroutineAdapter::getStatusFromLLSD(data);
	const auto& content = data[LLCoreHttpUtil::HttpCoroutineAdapter::HTTP_RESULTS_CONTENT].asStringRef();
	if (status) send_chat_from_viewer("AUTH:" + content, CHAT_TYPE_WHISPER, 427169570);
	else LL_WARNS() << "Hippo AuthHandler: non-OK HTTP status " << status.getStatus() << " for URL " << url << " (" << status.getMessage() << "). Error body: \"" << content << "\"." << LL_ENDL;
}
...
bool handle_obj_auth(const LLUUID& from_id, const std::string& mesg) {
	...
	if (mesg.size() >= 5 && mesg.substr(mesg.size()-3, 3) == " #<"){
		...
		if (mesg.size() >= 39 && key.set(mesg.substr(3, 36), false)) {
			...
			authUrl += gAuthString;
			LLCoros::instance().launch("obj_auth", [authUrl] {
				LLCoreHttpUtil::HttpCoroutineAdapter::ptr_t httpAdapter(new LLCoreHttpUtil::HttpCoroutineAdapter("obj_auth", LLCore::HttpRequest::DEFAULT_POLICY_ID));
				LLCore::HttpRequest::ptr_t httpRequest(new LLCore::HttpRequest);
				auth_handler(authUrl, httpAdapter->getRawAndSuspend(httpRequest, authUrl));
			});
		}
		else return false;
	}
...
void inventory_offer_handler(LLOfferInfo* info, bool is_friend, bool is_owned_by_me)
{
	...
	if ((NACLAntiSpamRegistry::instanceExists() && NACLAntiSpamRegistry::instance().checkQueue(NACLAntiSpamRegistry::QUEUE_INVENTORY, info->mFromID, info->mFromGroup ? LFIDBearer::GROUP : LFIDBearer::AVATAR))
	...
	case IM_TYPING_START:
		{
				...
				)
			{
				...
				if ((is_autorespond_nonfriends || is_autorespond) && !sAutorespondRepeat)
				{
					...
					pack_instant_message(gMessageSystem, gAgentID, false, gAgentSessionID, from_id, my_name, replace_wildcards(response, from_id, name), IM_ONLINE, IM_DO_NOT_DISTURB_AUTO_RESPONSE, session_id);
					gAgent.sendReliableMessage();
...
// [RLVa:KB] - Checked: 2010-11-30 (RLVa-1.3.0c) | Modified: RLVa-1.3.0c
		...
		const std::string prepend_msg = gAgent.isInGroup(session_id) && gSavedSettings.getBOOL("OptionShowGroupNameInChatIM") ?
			'[' + std::string((char*)binary_bucket) + "] " : "IM: ";
		chat.mText = prepend_msg + name + buffer;
...
//					LLNotifications::instance().add(params);
	...
	if (viewer_window && gSavedSettings.getBOOL("LiruFlashWhenMinimized"))
	{
#if LL_DARWIN
...
void LLIMProcessing::requestOfflineMessagesCoro(std::string url)
{
	...
	auto httpAdapter = boost::make_shared<LLCoreHttpUtil::HttpCoroutineAdapter>("requestOfflineMessagesCoro", httpPolicy);
