...
LLFloaterMessageBuilder::LLFloaterMessageBuilder(const std::string& initial_text)
:	LLFloater(),
	LLEventTimer(1.0f),
//	mNetInfoMode(NI_NET),
...
/*void LLFloaterMessageBuilder::onOpen(const LLSD& key)
{
	if(key.has("initial_text"))
	{
		...
		getChild<LLTextEditor>("message_edit")->setText(mInitialText);
	}
}*/
...
void LLFloaterMessageBuilder::onCommitPacketCombo(LLUICtrl* ctrl)
{
	...
	{
		getChild<LLTextEditor>("message_edit")->setText(LLStringUtil::null);
		return;
	}
	...
	getChild<LLTextEditor>("message_edit")->setText(text);
}
...
void LLFloaterMessageBuilder::onClickSend()
{
	...
	mMessageSender.sendMessage(end_point, getChild<LLTextEditor>("message_edit")->getText());
}
